...
function TableNumberModal() {
  ...
  return (
    <div className={`modal ${isOpen ? 'fixed inset-0 z-50 bg-zinc-800 bg-opacity-75 backdrop-blur-sm flex justify-center items-center' : 'hidden'}`}>
      <div className="modal-content bg-zinc-200 dark:bg-zinc-900 text-zinc-950 dark:text-gray-300 p-6 rounded-2xl flex-col text-center w-8/12 md:w-6/12 lg:w-5/12 xl:w-3/12 space-y-4">
        ...
        <div className="text-black">
          <input
            className={`placeholder:italic placeholder:text-slate-600 block bg-white w-full border-2 ${isValid ? 'border-sky-500' : 'border-red-500'} rounded-md py-2.5 px-3 shadow-sm focus:outline-none sm:text-sm`}
        ...
        <div>
          <button className={`font-semibold rounded-xl w-full py-2 border-2 hover:text-gray-300 dark:hover:text-slate-800 ${isValid ? 'border-sky-500 hover:bg-sky-500 ' : ' border-red-500 hover:bg-red-500'} duration-100`} onClick={handleSelect}>
            Select
          </button>
        </div>
      </div>
